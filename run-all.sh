...
# ── artifact collection ────────────────────────────────────────────────
if [ "${NO_ARTIFACTS:-0}" != "1" ]; then
  ART="${ARTIFACTS_DIR:-$CDIR/artifacts}"
  mkdir -p "$ART"
  tar -czf "$ART/jjlab-e2e-$RUN_ID.tar.gz" -C "$WORK" .
  {
    echo "jjlab-e2e run $RUN_ID @ $(date -u +%FT%TZ)"
    echo "base:      ${BASE:-http://jj-lab.temp.svc.cluster.local}"
    echo "write tok: ${WRITE_TOKEN:-devtoken}"
    echo "result:    $TOT_PASS passed, $TOT_FAIL failed, $TOT_SKIP skipped"
    echo "work dir:  $WORK"
    echo "artifact:  $ART/jjlab-e2e-$RUN_ID.tar.gz"
    echo ""
    echo "per-suite:"
    for s in "${SUITES[@]}"; do
      r=$(cat "$WORK/$s.result" 2>/dev/null || echo "? ? ?")
      printf "  %-14s %s\n" "$s" "$r"
    done
...
[ "$TOT_FAIL" -eq 0 ] && echo "JJLAB REGISTRY E2E GREEN"
[ "$TOT_FAIL" -eq 0 ]
